...
# Tab 4: Simulation
        with tab4:
            ...
            if not st.session_state.get('weights'):
                ...
                # Base simulation settings from your original code
                ...
                # --- BIAS SIMULATION UI (RESTORED AND FIXED) ---
                st.markdown("---")
                ...
                if enable_bias:
                    ...
                    with col3:
                        # This is the corrected slider widget
                    ...
                    # HELPER BOX: Restored from your original code
                    with st.expander("🔍 Bias Configuration Preview", expanded=True):
                        ...
                        comparison_df = pd.DataFrame({'Scale Value': list(range(1, 6)), 'Original': [example_original.get(i, 0) for i in range(1, 6)], 'Biased': [example_biased.get(i, 0) for i in range(1, 6)]})
                        fig = go.Figure()
                ...
                if st.button("🚀 Simulate Responses"):
                    with st.spinner(f"Simulating {num_simulations} responses..."):
                        ...
                        if enable_bias:
                            ...
                            updated_weights = apply_bias_to_weights(updated_weights, bias_type, bias_strength, bias_percentage)
                ...
                if st.session_state.get('sim_data') is not None:
                    ...
                    with st.expander("View simulated data", expanded=True):
                        ...
                        num_rows = st.slider("Number of rows to display", 5, min(total_rows, 1000), min(10, total_rows), key="sim_data_preview_slider")
                    ...
                    if st.session_state.get('show_stat_analysis', False):
                        try:
                            with st.expander("Descriptive Statistics Comparison", expanded=True):
                                ...
                                stats_diff = {metric: abs(real_desc[metric] - sim_desc[metric]).mean() for metric in ['mean', 'std', 'var', 'skew', 'kurtosis']}
                            ...
                            with st.expander("Distribution Comparison (Jensen-Shannon)", expanded=True):
                                js_distances = {}
                                for item in st.session_state.likert_items:
                                    if item in st.session_state.sim_data.columns:
                                        ...
                                        js_div = 0.5 * (np.sum(real_probs * np.log(real_probs / m_dist)) + np.sum(sim_probs * np.log(sim_probs / m_dist)))
                                ...
                                dist_similarity = divergence_df['Similarity (%)'].mean()
                                st.metric("Overall Distribution Similarity", f"{dist_similarity:.2f}%")

                            if st.session_state.get('clusters'):
                                with st.expander("Reliability Comparison (Cronbach's Alpha)", expanded=True):
                                    alpha_data = []
                                    valid_clusters = {sc: [i for i in items if i in st.session_state.sim_data.columns] for sc, items in st.session_state.clusters.items()}
                                    for sc, items in valid_clusters.items():
                                        if len(items) > 1:
                                            orig_alpha = st.session_state.alphas.get(sc, 0)
                                            sim_alpha = cronbach_alpha(st.session_state.sim_data, items)
                                            alpha_data.append({'Cluster': sc, 'Original Alpha': orig_alpha, 'Simulated Alpha': sim_alpha, 'Difference': abs(orig_alpha - sim_alpha)})
                                    if alpha_data:
                                        alpha_df = pd.DataFrame(alpha_data)
                                        alpha_df['Similarity (%)'] = 100 - (alpha_df['Difference'] * 100)
                            ...
                            overall_metrics = {'Descriptive': overall_similarity, 'Correlation': corr_similarity, 'Distribution': dist_similarity}
                            if 'reliability_similarity' in locals():
                                overall_metrics['Reliability'] = reliability_similarity
                            final_score = np.mean(list(overall_metrics.values()))
                            
                            fig = go.Figure(go.Indicator(
                                mode = "gauge+number+delta",
                                value = final_score,
                                title = {'text': "Overall Simulation Quality Score"},
                                gauge = {'axis': {'range': [None, 100]}, 'bar': {'color': "darkblue"},
                                         'steps': [{'range': [0, 70], 'color': 'lightgray'}, {'range': [70, 85], 'color': 'gray'}]}))
                            st.plotly_chart(fig, use_container_width=True)
